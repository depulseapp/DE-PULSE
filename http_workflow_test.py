...
try:
    ...
    html=req('/'); ck('html branding','DE.PULSE' in html); ck('html current release renderer',f'renderer.js?v={EXPECTED_VERSION}' in html and 'WC ·' not in html and 'test-build-badge' not in html); ck('start data label','START DATA' in html); ck('no global sidebar', 'data-page="global"' not in html); ck('no options desk','data-page="options"' not in html)
