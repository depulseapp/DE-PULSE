...
func newMarketDataQuoteAdapter(token string) marketDataQuoteAdapter {
	return marketDataQuoteAdapter{token: strings.TrimSpace(token), baseURL: marketDataDefaultBaseURL, client: &http.Client{Timeout: marketDataQuoteTimeout}}
}
...
func (a marketDataQuoteAdapter) fetchQuote(ctx context.Context, symbol string) (Quote, error) {
	...
	q := Quote{Symbol: symbol, Price: payload.Last[0], Bid: payload.Bid[0], Ask: payload.Ask[0], Volume: payload.Volume[0], Source: marketDataProviderName, PriceScale: 1, FeedType: "rest-shadow-delayed", DataState: "delayed", ProviderTimestamp: payload.Updated[0], IngestedAt: time.Now().Unix()}
	return finalizeQuote(q), nil
}

func marketDataShadowRegistration() ProviderRegistration {
	return ProviderRegistration{
		Name: marketDataProviderName, QuotaLabel: "Provider entitlement / request quota dependent", CostClass: "Provider plan dependent",
		Configured: func(_ Settings, s Secrets) bool { return strings.TrimSpace(s.MarketData) != "" },
		ConfigurationFingerprint: func(_ Settings, s Secrets) [32]byte { return providerConfigurationFingerprint(marketDataProviderName, strings.TrimSpace(s.MarketData)) },
		Routes: []ProviderDatasetContract{{
			Dataset: "US Live Equities", Capability: "Delayed U.S. stock quote validation", Priority: 100, Lifecycle: providerlifecycle.Shadow,
			ContractVersion: "marketdata-shadow-quote-v19.1.0", CanonicalOwner: "Quote", Consumer: "Adaptive Provider Registry shadow evaluation",
			...
			EvidenceRef: "governance/work-slices/ADAPT-V19-1-CANONICAL-FOUNDATION-001/closure.json#APR-03", ApprovalRef: "APR-03 exact-head qualification; lifecycle remains SHADOW",
