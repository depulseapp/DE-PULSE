#!/usr/bin/env bash
# DE.PULSE v18.9.0 exact-source G12 certification.
# TradeInsight Adaptive Provider Integration.
set -euo pipefail
...
evidence_root="${DEPULSE_EVIDENCE_DIR:-$repo_root/.depulse-certification/v18.9.0/$source_sha}"
...
echo "DE.PULSE v18.9.0 G12 exact-source certification"
echo "Source SHA: $source_sha"
echo "Branch: $source_branch"
echo "Started: $started_at"

echo
echo "[G0/G1] Canonical v18.9.0 release identity and issue #61 scope"
python3 release_identity.py --verify
python3 version_consistency_test.py
python3 - <<'PY'
import json
identity=json.load(open('release_identity.json'))
contract=json.load(open('release/v18.9.0/release_contract.json'))
assert identity['version']=='18.9.0'
assert identity['previous_stable']=='v18.8.2'
assert identity['stable_baseline']=='v18.8.2'
assert contract['release']=='18.9.0'
assert contract['certifiedStableBaseline']=='v18.8.2-stable'
assert contract['identity_asset']=='release-identity-v18.9.0.js'
assert 'ADAPT-TRADEINSIGHT-001' in contract['scope']
assert 'DIRECT_SEC_EDGAR_AUTHORITATIVE_FOR_FORM4' in contract['protectedBoundaries']
print('PASS: v18.9.0 identity / release contract / bounded TradeInsight scope')
PY
...
echo
echo "[G2/G7/G8/G10/G12] TradeInsight v18.9 proof plus inherited v18.8 trust coverage"
go test -count=1 -run 'TestV189|TestV1882|TestV1881|TestV188' ./...
...
payload = {
    ...
    'tradeinsight_daily_history': 'PASS',
    'tradeinsight_bulk_history_gate': 'PASS',
    'tradeinsight_congress_shadow': 'PASS',
    'tradeinsight_optional_degradation': 'PASS',
    'tradeinsight_form4': 'GATED_EXACT_REST_CONTRACT_REQUIRED',
    'tradeinsight_top_movers': 'GATED_EXACT_REST_CONTRACT_REQUIRED',
    'tradeinsight_ticker_search': 'GATED_EXACT_REST_CONTRACT_REQUIRED',
    'direct_sec_edgar_form4_authority': 'PRESERVED',
    'smart_provider_router_v2_ownership': 'PRESERVED',
    'canonical_freshness_cache_persistence_ownership': 'PRESERVED',
    'go_full': 'PASS',
    'race': 'PASS',
    'randomized': 'PASS',
...
echo
echo "PASS: exact-source v18.9.0 G12 certification completed."
