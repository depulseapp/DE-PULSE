#!/usr/bin/env bash
# DE.PULSE v18.8.0 exact-source G12 certification.
# Shared Intelligence Consolidation + release-identity recovery proof.
set -euo pipefail
...
evidence_root="${DEPULSE_EVIDENCE_DIR:-$repo_root/.depulse-certification/v18.8.0/$source_sha}"
...
echo "DE.PULSE v18.8.0 G12 exact-source certification"
echo "Source SHA: $source_sha"
echo "Branch: $source_branch"
echo "Started: $started_at"

echo
echo "[G0/G1] Canonical release identity and frozen v18.8 scope"
python3 release_identity.py --verify
python3 version_consistency_test.py
python3 - <<'PY'
import json
scope=json.load(open('v18_8_0_scope.json'))
g=json.load(open('v18_8_0_g0_g3_contract.json'))
r=json.load(open('release/v18.8.0/release_contract.json'))
assert scope['version']=='18.8.0' and scope['status']=='G1_FROZEN'
assert scope['stableBaseline']=='v18.7.0-stable'
assert g['version']=='18.8.0' and g['g1']['status']=='FROZEN'
assert g['g0']['certifiedStable']=='v18.7.0-stable'
assert r['release']=='18.8.0'
assert r['certifiedStableBaseline']=='v18.7.0-stable'
assert r['identity_asset']=='release-identity-v18.8.0.js'
print('PASS: v18.8.0 frozen scope / G0-G3 / release contract')
PY

echo
echo "[G2/G7/G10] CI, reproducibility, portability, provider/data utility and rights"
...
echo
echo "[G2/G7/G8/G10/G12] v18.8 shared-universe consolidation proof"
go test -count=1 -run '^TestV188' ./...
...
echo
echo "[G9/G10/G12] Renderer syntax and deterministic/owner regressions"
...
payload = {
    ...
    'shared_us_equity_universe_v1880': 'PASS',
    'scanner_radar_refresh_coalescing': 'PASS',
    'stale_timestamp_truth': 'PASS',
    'bounded_retry_seed_fallback': 'PASS',
    'scanner_ranking_radar_promotion_invariants': 'PASS',
    'go_full': 'PASS',
    'race': 'PASS',
    'randomized': 'PASS',
...
echo
echo "PASS: exact-source v18.8.0 G12 certification completed."
