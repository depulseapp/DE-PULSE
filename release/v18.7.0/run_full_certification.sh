#!/usr/bin/env bash
# DE.PULSE v18.7.0 exact-source G12 certification.
# Current-source certification for Runtime Reliability & Data Truth.
set -euo pipefail
...
evidence_root="${DEPULSE_EVIDENCE_DIR:-$repo_root/.depulse-certification/v18.7.0/$source_sha}"
...
echo "DE.PULSE v18.7.0 G12 exact-source certification"
echo "Source SHA: $source_sha"
echo "Branch: $source_branch"
echo "Started: $started_at"

echo
echo "[G0/G1] Canonical release identity and frozen v18.7 scope"
python3 release_identity.py --verify
python3 version_consistency_test.py
python3 - <<'PY'
import json
scope=json.load(open('v18_7_0_scope.json'))
g=json.load(open('v18_7_0_g0_g3_contract.json'))
r=json.load(open('release/v18.7.0/release_contract.json'))
assert scope['version']=='18.7.0' and scope['status']=='G1_FROZEN'
assert g['version']=='18.7.0' and g['g1']['status']=='FROZEN'
assert r['release']=='18.7.0'
assert r['certifiedStableBaseline']=='v18.6.1-stable'
print('PASS: v18.7.0 frozen scope / G0-G3 / release contract')
PY
...
echo
echo "[G7/G8/G10/G12] Go format, vet, runtime reliability, full suite, race and randomized order"
...
go vet ./...
go test -count=1 -run '^TestV1870' ./...
go test -count=1 ./...
go test -race -count=1 ./...
go test -shuffle=on -count=2 ./...

echo
echo "[G9/G10/G12] Renderer syntax and current deterministic/owner regressions"
...
payload = {
    ...
    'runtime_reliability_v1870': 'PASS',
    'go_full': 'PASS',
    'race': 'PASS',
    'randomized': 'PASS',
...
echo
echo "PASS: exact-source v18.7.0 G12 certification completed."
