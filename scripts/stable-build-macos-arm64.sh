#!/bin/bash
set -euo pipefail
R="$(cd "$(dirname "$0")/.."&&pwd)"; S="$R/input/De-Pulse-v18.0.6-STABLE-Source.zip"; SHA='857c7999891225f5ac8dc26eaf1f2018dfd72ccbf64ef23b11a103e169679199'; FP='5f5285071f4a46ed6ab83026c158d58936eecdea38961940622faa8cfe4d7f90'; B='v18.0.6-stable-smart-provider-router-rapid-move-market-shock-hardening-20260814'; O="$R/out/macos-arm64"; W="$R/.work/macos-arm64"; A="$O/De-Pulse.app"; Z="$O/De-Pulse-v18.0.6-STABLE-macOS-Apple-Silicon.zip"
...
python3 v18_0_6_stable_promotion_gate.py; python3 v18_0_6_scope_gate.py; python3 v18_baseline_gate.py; CGO_ENABLED=1 go test -count=1 ./...; CGO_ENABLED=1 GOOS=darwin GOARCH=arm64 go build -trimpath -ldflags='-s -w' -o "$A/Contents/MacOS/DePulse-arm64" .; cp platform-icons/DePulse.icns "$A/Contents/Resources/DePulse.icns"
...
import json,sys
j=json.load(open(sys.argv[1])); assert j.get('version')=='18.0.6' and j.get('buildId')==sys.argv[2]
PY
