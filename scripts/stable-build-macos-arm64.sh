#!/bin/bash
set -euo pipefail
R="$(cd "$(dirname "$0")/.."&&pwd)"; S="$R/input/De-Pulse-v18.0.5-STABLE-Source.zip"; SHA='a31cc78184bd1b8b1f75a7ac5d3c8f2a1e8623a049bfd328226235f36c511ebd'; FP='6328baedc2358acb8280a1da4243f2ad02fe0694d3504a3a80c276e139eb268e'; B='v18.0.5-stable-ui-ux-symbol-management-hardening-20260814'; O="$R/out/macos-arm64"; W="$R/.work/macos-arm64"; A="$O/De-Pulse.app"; Z="$O/De-Pulse-v18.0.5-STABLE-macOS-Apple-Silicon.zip"
...
python3 v18_0_5_stable_promotion_gate.py; python3 v18_0_5_scope_gate.py; python3 v18_baseline_gate.py; CGO_ENABLED=1 go test -count=1 ./...; CGO_ENABLED=1 GOOS=darwin GOARCH=arm64 go build -trimpath -ldflags='-s -w' -o "$A/Contents/MacOS/DePulse-arm64" .; cp platform-icons/DePulse.icns "$A/Contents/Resources/DePulse.icns"
...
import json,sys
j=json.load(open(sys.argv[1])); assert j.get('version')=='18.0.5' and j.get('buildId')==sys.argv[2]
PY
