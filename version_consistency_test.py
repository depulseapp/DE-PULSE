...
boot=(ROOT/'app_bootstrap.go').read_text(); renderer=(ROOT/'renderer/renderer.js').read_text(); version=(ROOT/'VERSION.txt').read_text(); index=(ROOT/'renderer/index.html').read_text()
...
if patch_contract:
    ...
    need(str(cert.get('version'))==str(patch_contract.get('inherited_certification_plan')),'inherited certification plan mismatch')
    need(str(ci.get('version'))==str(patch_contract.get('inherited_ci_plan')),'inherited CI plan mismatch')
    need(patch_contract.get('quality_rule','').find('no gate')>=0,'patch quality inheritance rule missing')
else:
...
if errs:
    ...
    raise SystemExit(2)
print(f'Version consistency: PASS · v{VERSION} canonical release identity aligned; patch releases explicitly inherit the unchanged full certification/CI plans.')
