...
**Entry condition:** v18.9.x final closure must be zero-gap enough to serve as the trusted acquisition/identity/telemetry foundation. v19 does not redo the router or create new provider-specific owners; it measures and professionalizes the data plane created in v18.
...
2. **Provider quality / cost / coverage / SLO scorecards.** Measured freshness, completeness, latency, reliability, rate-limit pressure, cost/value, contribution/usefulness and fallback quality using v18.9 telemetry.
3. **Data reconciliation + disagreement + historical-quality hardening.** Source independence, conflict/reconciliation policy, corporate-action/adjustment correctness, historical depth/gaps and point-in-time provenance.
...
7. **ADR-GDI professional reliability hardening.** Capability SLOs, degradation history, provider/DB/runtime reliability scorecards, restart/warm-start, query/index/pool/capacity tuning, load shedding, bounded operating limits and cost/value efficiency.
8. **Specialized/paid-provider gap evaluation.** Only consider replacement/additional paid data where measured v19 evidence proves a material capability/quality/rights gap; integrate through the same Smart Provider Router contracts, never by special path.
...
11. **ADR-GDI adaptive optimization.** Governed SHADOW/Champion-Challenger learning for provider recovery prediction, cooldown/backoff, workload priority, fallback usefulness and capacity policy; cannot self-promote.
...
- **v18.9.x = trustworthy plumbing and truth:** stable runtime, Settings, coverage-aware acquisition, canonical identity, Market Data Modes, useful TradeInsight evidence, efficiency telemetry.
- **v19 = professional measurement and evidence infrastructure:** prove which sources are useful/reliable/lawful, reconcile disagreement, harden 13F/TDTI/AODR/ADR-GDI evidence and build point-in-time outcome lineage.
...
This ordering prevents v20 from learning provider artifacts, stale/partial data, survivorship leakage or undocumented provenance. It also prevents v19 from creating a second router or duplicating the adaptive acquisition work completed in v18.9.x.
