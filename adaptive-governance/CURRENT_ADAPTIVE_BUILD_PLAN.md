...
`stabilize -> establish canonical owners -> instrument -> validate in SHADOW -> expand -> operationalize -> close -> establish hosted control/privacy/environment plane -> establish recoverable persistence/secrets/supply chain -> activate hosted data plane -> synchronize -> prove parity -> harden -> build evidence substrate -> learn`
...
Define and enforce the lifecycle of account/user/device and hosted research data before shared schema/retention behavior freezes. Required scope: authoritative data inventory/classification, purpose/minimization, retention/deletion schedules, user/admin review/export, account deactivation/deletion, device retirement, SQLite/PostgreSQL/sync-log/tombstone/cache/backup deletion or cryptographic-expiry semantics, audit/log retention, operator/support access controls, legal-hold exception semantics if ever required, and explicit data-residency/region disposition. Privacy lifecycle must compose with provider legal rights and product retention needs rather than silently overriding either.

#### v19.0.4 — Hosted Environment / IaC / Service Trust Foundation
Establish versioned desired state for hosted infrastructure before production data-plane activation: isolated development/test/stage/production environments, environment-specific identities/secrets/config, least-privilege service identities, network/service trust boundaries, ingress/egress policy, TLS/certificate ownership, infrastructure-as-code or equivalent reproducible configuration, drift detection, change review, reproducible deploy/rollback, safe configuration promotion and environment-level audit/observability. No manually maintained production snowflake environment.

#### v19.0.5 — PostgreSQL Tenancy / Schema / Pool / Recovery Foundation
Shared authority only: tenant/account isolation, schema/migration ownership, connection pools/capacity, indexes, encrypted backup, HA/failover, PITR, restore drills and frozen RPO/RTO. Use expand/contract migration strategy where practical. Schema/retention decisions must consume v19.0.3 privacy/data-governance policy. No broad sync activation.

#### v19.0.6 — Managed Secrets / KMS Lifecycle
Server-side provider/service-secret ownership, environment separation, least privilege, versioned references, rotation, rollback, compromise revoke, redaction and auditable administration. Commercial clients contain no platform provider secret. Secret ownership must compose with v19.0.4 environment/service identities.

#### v19.0.7 — Software Supply-Chain / Artifact & Dependency Assurance
Create executable software/component provenance before hosted activation: dependency/component inventory, direct + transitive dependency visibility, vulnerability/advisory scanning, SBOM generation where applicable, source/license policy, approved registries/sources, lockfile/build reproducibility, artifact signing/attestation or equivalent integrity proof, container/binary provenance where used, vulnerable-component patch/revocation procedure, and deployment linkage from source -> build -> artifact -> environment. No untraceable hosted binary/container/dependency enters production.
...
**v19.0 exit:** provider rights, tenant identity, product entitlement, privacy/data lifecycle, hosted environment/IaC trust, PostgreSQL recovery, secrets, software-supply-chain provenance, SLOs and data quality are executable/evidenced before shared hosted activation.

### v19.1.x — Zero-Key Provider Data Plane + Native Sync Foundation

#### v19.1.0 — Authenticated Hosted Provider Gateway
Expose the existing Smart Provider Router v2 through a versioned hosted API boundary. Reuse canonical freshness/cache/persistence/state, resolve provider credentials server-side only, add bounded rate/backpressure/circuit/kill-switch/degraded behavior, and maintain API inventory/version/deprecation ownership. Gateway deployments must consume the v19.0 environment/IaC and supply-chain foundations. No second provider stack.

#### v19.1.1 — Unified Serving Policy + Live Fan-Out Isolation
Enforce tenant/RBAC/product-entitlement/provider-right/privacy-data-class checks across router/cache/persistence/REST/WebSocket/SSE. Existing multi-feed allocator remains upstream subscription owner. Prevent cross-tenant, premium-tier, legally restricted or privacy-policy-incompatible data leakage.

#### v19.1.2 — Sync Protocol Foundation
Stable IDs, schema/domain capability versions, snapshot/high-watermark bootstrap, SQLite atomic outbox, authenticated idempotent push, authoritative server sequence/change log, incremental pull, durable per-device checkpoint, tombstones, retention/compaction, stale-device re-bootstrap and mixed-version negotiation. Sync retention/deletion semantics consume v19.0.3. Client wall clock never decides conflicts alone. No raw SQLite/PostgreSQL replication.

#### v19.1.3 — macOS Preferences + Watchlist Pilot
First real sync-domain activation. Prove portable preferences/ticker convergence, offline writes, restart/reconnect, conflicts, user switching, local account isolation, lost/revoked-device behavior, account deactivation/deletion semantics applicable to local state, and no destructive reset of `PersonalMarketTerminal`.
...
**v19.1 exit:** zero-key gateway, unified serving policy and proven native sync transport/macOS pilot without protected-session degradation or privacy/retention bypass.

### v19.2.x — Cross-Platform Account Parity & Hosted Assurance

#### v19.2.0 — Windows x64 Sync Parity
Same protocol/state/security/privacy semantics as macOS; no Windows-specific account truth/provider path.

#### v19.2.1 — Hosted Web Account Parity
Same authenticated APIs, session/capability/product-entitlement/privacy truth and PostgreSQL account state. Browser cache is non-authoritative. Direct-route/API authorization and web session/cookie/security-header controls are evidenced.

#### v19.2.2 — Rights-Aware Research / Durable State Portability
Sync only product-owned, lawful, entitled, provenance-bound durable research/state consistent with data-governance/retention policy. Preserve as-observed/revision history where required. Live market truth remains canonical freshness/provider/state-owned.

#### v19.2.3 — Tenant-Aware Metering / Cost / Usage Observability
Per tenant/account/user/device/capability attribution, plan/quota consumption, cache/call avoidance, stream usage, provider cost where known, tenant health, throttling and anomalous consumption visibility. Telemetry/log retention follows v19.0.3 and must not leak secrets or excess user data.

#### v19.2.4 — Multi-User Security / Abuse / Capacity Hardening
Object/function-level authorization negatives, sensitive-flow abuse protection, rate limits, fairness, noisy-neighbor isolation, queue/circuit/load shedding, edge protection, DB/pool limits and protected-session capacity. Include environment-boundary and deployment/configuration negative tests.

#### v19.2.5 — Hosted Sync / Gateway Assurance Closure (#66)
No feature scope. Failure/adversarial matrix: cross-account denial, role/session/device revocation, duplicate replay, network loss during apply, checkpoint expiry/re-bootstrap, provider-right downgrade/expiry, product-plan downgrade/suspension, account export/deactivation/deletion/retention behavior, secret rotation rollback, DB failover/PITR/restore, mixed-version clients, environment/config drift, artifact/dependency provenance failure, vulnerable-component response, queue pressure and protected-session load. #66 closes only on executable Mac/Windows/web parity plus zero-key/rights/product-entitlement/privacy/sync/DR/environment/supply-chain proof.
...
Capability SLO/error-budget/degradation history, provider/DB/runtime scorecards, restart/warm-start, query/index/pool tuning, load shedding, operating limits, reserve sizing, maintenance/preemption economics and hosted incident/runbook readiness. Runbooks include environment recovery, configuration rollback, dependency/artifact compromise response and privacy/account-data operations.
...
No model scope. Prove point-in-time features/outcomes, provenance, rights, privacy/retention compatibility, independence, synchronized-evidence safety, leakage controls and reliability history are adequate for adaptive research.

### v19.5.0 — v19 Major Closure
No feature scope. Whole-system Principal Engineer/security/privacy/data/operational/commercial-readiness audit. Require #66 closure, tenant isolation, product-entitlement/provider-right separation, account-data lifecycle/export/deletion evidence, hosted environment/IaC drift and rollback proof, software-supply-chain/SBOM/provenance assurance, API inventory/version compatibility, tested rollback/recovery, actual supported runtime/package/deployment evidence, SLO/capacity truth and zero unresolved P0 architecture gap.
...
- **v20.0.0 — Adaptive Research Control Plane + Immutable Experiment Ledger.** Dataset/version lineage, feature/provenance snapshots, cohorts, reproducibility, leakage controls, retention/privacy compatibility, promotion/rollback evidence.
...
`v18.9 stabilize/instrument/validate -> v19.0 control/privacy/environment/data foundation -> v19.1 hosted data plane/sync -> v19.2 parity/assurance -> v19.3 evidence substrate -> v19.4 reliability/readiness -> v19.5 closure -> v20 governed adaptive research`
...
- **G2:** canonical-owner map, tenant/data classification, privacy/purpose/minimization scope, threat-model scope and ADR for material irreversible decisions;
- **G3:** API/schema/protocol contracts, API inventory/deprecation, data-retention/deletion/export contract, environment/IaC/service-trust design, dependency/SBOM/provenance policy, migration/rollback, compatibility matrix, SLO/error-budget targets, observability, failure-injection and rollout plan;
- **G4:** implementation + unit/contract tests + reproducible configuration/build evidence + feature/kill-switch controls where useful;
- **G7:** negative authorization/tenant isolation, product-entitlement, provider-right, privacy/data-access/deletion, secret/redaction and adaptive-governance proof;
- **G8:** load/soak/capacity, fairness/noisy-neighbor, backpressure, chaos/failure recovery, DB/pool/provider limits, environment/config failure behavior and protected-session proof;
- **G9:** role-aware Mac/Windows/web UX, data-export/deletion/status UX where applicable, and direct-route/API denial consistency;
- **G10:** production-readiness reconciliation; unresolved P0 threat/privacy/rights/supply-chain/environment/recovery/compatibility/duplicate-owner issue blocks freeze;
- **G11/G12:** immutable RC/full certification;
- **G13/G14:** native/hosted artifact provenance, dependency/SBOM/attestation where applicable, deployment identity and runtime proof;
- **G15:** controlled/canary/progressive promotion evidence for hosted-risk changes where applicable, including environment/config/dependency rollback and kill criteria;
- **G16:** implementation-miss review, incidents/metrics, privacy/data-lifecycle and supply-chain findings, cleanup and exact next handoff.
