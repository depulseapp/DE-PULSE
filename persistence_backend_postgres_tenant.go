...
func (b *hostedTenantPostgresBackend) migrateLegacyHostedTenantState(ctx context.Context) error {
	...
	if tenantRows == 0 {
		...
		// Once tenant-owned rows validate, the old aggregate row is no longer an
		// authority. The legacy table remains for expand/contract compatibility.
...
func (b *hostedTenantPostgresBackend) writeHostedTenantIdentityState(ctx context.Context, state IdentityPersistentState, allowLegacy bool) error {
	...
	// The v3 aggregate table is retained as an expand/contract schema artifact,
	// but its cross-tenant payload is retired once the tenant write commits.
