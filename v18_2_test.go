...
func TestV182AdminRoleHierarchyAndLifecycle(t *testing.T) {
	...
	if owner.Role != RoleOwner { t.Fatalf("expected OWNER, got %s", owner.Role) }
...
func TestV182PresenceUsesCanonicalSessionAndSSETouch(t *testing.T) {
	...
	for _, u := range users {
		if u.ID == user.ID {
			found = true
			if u.Presence != "ACTIVE" || u.ActiveSessionCount != 1 { t.Fatalf("unexpected initial presence: %+v", u) }
		}
	}
	if !found { t.Fatal("user missing from admin snapshot") }
	for _, rec := range sessions {
		if rec.UserID == user.ID && (rec.Presence != "ACTIVE" || rec.ID != up.SessionID) { t.Fatalf("unexpected session view: %+v", rec) }
	}

	s.now = func() time.Time { return base.Add(2 * time.Minute) }
	users, _, _ = s.adminSnapshot(owner)
	for _, u := range users {
		if u.ID == user.ID && u.Presence != "IDLE" { t.Fatalf("expected IDLE after inactivity: %+v", u) }
	}
	s.touchSessionID(up.SessionID)
	users, _, _ = s.adminSnapshot(owner)
	for _, u := range users {
		if u.ID == user.ID && u.Presence != "ACTIVE" { t.Fatalf("SSE/session touch did not restore ACTIVE: %+v", u) }
	}
}
...
func TestV182PasswordResetDisableAndSessionRevocation(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := s.adminResetPassword(owner, user.ID, "replacement temporary password"); err != nil { t.Fatal(err) }
	if _, err := s.resolve(token, false); err == nil { t.Fatal("password reset did not revoke prior session") }
	newToken, newPrincipal, err := s.authenticate(user.Username, "replacement temporary password")
	if err != nil { t.Fatal(err) }
	if !s.userRequiresPassword(user.ID) { t.Fatal("admin reset must require password change") }
	if err := s.adminRevokeSession(owner, newPrincipal.SessionID); err != nil { t.Fatal(err) }
	if _, err := s.resolve(newToken, false); err == nil { t.Fatal("admin session revoke did not invalidate session") }

	token2, _, err := s.authenticate(user.Username, "replacement temporary password")
	if err != nil { t.Fatal(err) }
	if err := s.adminSetUserStatus(owner, user.ID, UserDisabled); err != nil { t.Fatal(err) }
	if _, err := s.resolve(token2, false); err == nil { t.Fatal("disabled user session remained valid") }
	if _, _, err := s.authenticate(user.Username, "replacement temporary password"); err == nil { t.Fatal("disabled user could authenticate") }
	_ = principal
}
...
func TestV182AdminViewsRedactCredentialMaterial(t *testing.T) {
	...
	for _, forbidden := range []string{"passwordhash", "tokenhash", "argon2id", "redaction temporary password"} {
		if strings.Contains(text, forbidden) { t.Fatalf("admin view leaked credential material %q", forbidden) }
	}
}
