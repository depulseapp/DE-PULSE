...
func hostedMFACredentialView(rec HostedMFACredentialRecord) HostedMFACredentialView {
	return HostedMFACredentialView{
		ID: rec.ID, Label: rec.Label, Algorithm: rec.Algorithm, CreatedAt: rec.CreatedAt,
...
func hostedMFASigningPayload(challengeID string, principal Principal, credentialID, nonce string) []byte {
	return []byte(strings.Join([]string{
		...
		normalizedTenantID(principal.TenantID),
...
func hostedMFASessionMatches(rec SessionRecord, principal Principal, now int64) bool {
	return rec.ID == principal.SessionID && rec.UserID == principal.UserID &&
		normalizedTenantID(rec.TenantID) == normalizedTenantID(principal.TenantID) &&
		rec.RevokedAt == 0 && now < rec.IdleExpiresAt && now < rec.AbsoluteExpiresAt
}
...
func (s *IdentityService) enrollHostedMFACredential(principal Principal, label, encodedPublicKey string) (HostedMFACredentialRecord, error) {
	...
	for i := range s.state.Users {
		...
		credential := HostedMFACredentialRecord{
			ID: randomID("mfa"), Label: label, Algorithm: hostedMFAAlgorithmEd25519,
			PublicKey: canonicalKey, CreatedAt: now,
		}
...
func (s *IdentityService) createHostedMFAChallenge(principal Principal, credentialID string) (HostedMFAChallenge, error) {
	...
	rec.MFAChallenge = &HostedMFAChallengeRecord{
		ID: challengeID, CredentialID: selected.ID, ChallengeHash: hostedMFAChallengeHash(payload),
	...
	return HostedMFAChallenge{
		ID: challengeID, CredentialID: selected.ID, Algorithm: selected.Algorithm,
		SigningPayload: base64.RawURLEncoding.EncodeToString(payload), ExpiresAt: expiresAt,
	}, nil
}
