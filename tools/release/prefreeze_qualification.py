...
def fingerprint():
    return canonical_source_fingerprint(
        ROOT,
        excluded_dirs=FINGERPRINT_EXCLUDED_DIRS,
        excluded_suffixes=FINGERPRINT_EXCLUDED_SUFFIXES,
    )
...
def wave(name, jobs, max_workers, outdir):
    ...
    if max_workers == 1:
        for job in jobs:
            result = run_job(job, outdir)
            results.append(result)
            state = 'RESUME' if result.get('reused') else result['status']
            print(f"  [{state}] {result['id']} · {result['duration_seconds']}s", flush=True)
...
def write_summary(outdir, fp, rows, status, started):
    ...
    fresh = sum(r['duration_seconds'] for r in rows if not r.get('reused'))
    wall = time.time() - started
    ratio = round(fresh / max(wall, 0.001), 2) if fresh else 0.0
    summary = {
        'schema': 'DE.PULSE-PREFREEZE-QUALIFICATION-2',
        'generated_at': dt.datetime.now(dt.timezone.utc).isoformat(),
        'source_fingerprint': fp,
        'status': status,
        'wall_seconds': round(wall, 3),
        'evidence_summed_job_seconds': round(total, 3),
        'fresh_job_seconds': round(fresh, 3),
        'reused_count': sum(1 for r in rows if r.get('reused')),
        'current_run_parallel_efficiency_ratio': ratio,
        'resume_protocol': 'adaptive-governance/BUILD_RESUME_PROTOCOL.md',
        'checkpoint_metadata_fingerprint_excluded': True,
        'results': rows,
    }
...
def main():
    fp = fingerprint()
    outdir = ROOT.parent / f'.depulse-prefreeze-{fp[:12]}'
    ...
    light = [
        ...
        ('content', [py, 'tools/ci/content_copy_audit_test.py'], 60),
    ...
    if any(r['status'] != 'PASS' for r in rows):
        write_summary(outdir, fp, rows, 'FAIL', started)
        return 2
    ...
    summary = write_summary(outdir, fp, rows, status, started)
    print(
        f"Pre-freeze qualification: {status} · wall {summary['wall_seconds']:.1f}s · "
        f"evidence jobs {summary['evidence_summed_job_seconds']:.1f}s · "
        f"fresh jobs {summary['fresh_job_seconds']:.1f}s · reused {summary['reused_count']} · "
        f"current-run concurrency ratio {summary['current_run_parallel_efficiency_ratio']}x"
    )
