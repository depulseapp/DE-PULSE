...
try {
  ...
  $proc=Start-Process -FilePath $pkgExe -PassThru
  try {
    $instance=Join-Path $profile "$runtimeConfig/instance.json"
    for($i=0;$i -lt 180 -and -not (Test-Path $instance);$i++){ Start-Sleep -Milliseconds 100 }
    if(-not (Test-Path $instance)){ throw 'packaged Windows app did not create instance.json' }
    ...
    if($null -eq $health){ throw 'health endpoint unavailable' }
    if($health.version -ne $env:DEPULSE_VERSION -or $health.buildId -ne $env:DEPULSE_BUILD_ID){ throw 'runtime identity mismatch' }
    $ready=$null
    ...
    if($null -eq $ready -or -not $ready.ok -or $ready.persistence.backend -ne 'sqlite' -or -not $ready.persistence.ready){ throw 'SQLite readiness failed' }
    $rootResponse=Invoke-WebRequest -Uri "$base/" -TimeoutSec 2 -UseBasicParsing
    if($rootResponse.StatusCode -ne 200 -or $rootResponse.Content.Length -eq 0){ throw 'root surface failed' }
    $db=Join-Path $profile "$runtimeConfig/depulse-v17.db"
    if(-not (Test-Path $db)){ throw 'SQLite database missing' }
    python -c "import sqlite3,sys; c=sqlite3.connect(sys.argv[1]); assert c.execute('pragma integrity_check').fetchone()[0]=='ok'; v=[r[0] for r in c.execute('select version from schema_migrations order by version')]; assert v and v==sorted(set(v)),v; assert c.execute('select count(*) from symbol_registry').fetchone()[0]>0; assert c.execute('select count(*) from identity_state').fetchone()[0]>=1; print('Packaged Windows SQLite: PASS migrations=%s'%v); c.close()" $db
    if($LASTEXITCODE -ne 0){ throw 'SQLite audit failed' }
    Write-Host 'Packaged Windows runtime health/readiness/root: PASS'
  }
  finally {
  ...
  $evidence=[ordered]@{
    schema='DE.PULSE-G13-G14-NATIVE-2'; release="v$($env:DEPULSE_VERSION)"; platform='Windows x64'; status='PASS';
    ...
    checks=[ordered]@{exactGitObjectFingerprint='PASS';nativeBuild='PASS';peX64Format='PASS';winsqlite3Runtime='PASS';cleanExtraction='PASS';actualPackagedLaunch='PASS';healthIdentity='PASS';readySQLite='PASS';sqliteMigrations='PASS';sqliteIntegrity='PASS';rootSurface='PASS'};
    generatedAt=[DateTime]::UtcNow.ToString('o')
  }
  $evidence | ConvertTo-Json -Depth 6 | Set-Content -Encoding UTF8 (Join-Path $out 'G13-G14-Windows-x64.json')
  Write-Host "PASS: G13/G14 Windows x64 v$($env:DEPULSE_VERSION)"
}
finally {
  Pop-Location
}
